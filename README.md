...
# Testing
You may use `infer_single_random_gate.py` to validate `best.py` on a single random gate given a mav sim instance is running. Currently, the code expects `best.py` to be located somewhere in `~\mav_sim\runs\pose` and outputs the code to `~/mav_sim/test_projections`.